...
def sample_white_balance(decoder: RawDecoder, raw_path: Path, x: int, y: int) -> list[float]:
    ...
    averages a small patch around `(x, y)` (reference space, same as
    `FrameResult`), and returns `[R, G1, B, G2]` multipliers — normalized to
    G=1 — that make that patch neutral. Meant to be sampled once, from a
    neutral point (the light table background) in the session's first
    captured image, and reused as-is for the rest of the session: the
    ...
    r, g, b = (max(float(patch[:, :, channel].mean()), _MIN_CHANNEL_VALUE) for channel in range(3))
