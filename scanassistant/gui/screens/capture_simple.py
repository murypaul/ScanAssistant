...
frame to edit or a positive/master preview to render: the inherited
ingestion loop, live view, accept/reject/rename, name-conflict panel, and
warning/critical banners already behave correctly with no code of their
own. This class only removes the controls that would otherwise sit there
doing nothing (crop editing, positive/master preview, manual rotation,
white balance picking) and adds the one thing full mode doesn't need on a
dedicated key: renaming the current image on the fly.
"""
...
class SimpleCaptureScreen(CaptureScreen):
    ...
    # --- crop/positive/rotation/white-balance: nothing to do in this mode --
